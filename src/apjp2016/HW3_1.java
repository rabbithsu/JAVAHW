...
public class HW3_1 {
	...
	public static List<String> sortWordsByLength(List<String> nWords) {
		...
		//Collections.sort(List<T> nWords, Comparator<? super T>);
		
		for(int i = 0; i < nWords.size()-1; i++){
			for(int j = 0; j < nWords.size()-i-1; j ++){
				if(nWords.get(j).length() > nWords.get(j+1).length()){
					String tmp = nWords.get(j);
					nWords.set(j, nWords.get(j+1));
					nWords.set(j+1, tmp);
				}else if(nWords.get(j).length() == nWords.get(j+1).length()){
					if(nWords.get(j).compareTo((nWords.get(j+1))) > 0){
						String tmp = nWords.get(j);
						nWords.set(j, nWords.get(j+1));
						nWords.set(j+1, tmp);
					}
	...
	public static List<String> sortWordsByNumberOfChar(List<String> nWords, char... cs) {
		...
		int[] count = new int[nWords.size()];
		for(int i = 0; i < nWords.size(); i++){
			for(char a : nWords.get(i).toCharArray()){
				for(char c : cs){
					if(c == a){
						count[i]++;
					}
